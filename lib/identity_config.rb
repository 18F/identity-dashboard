...
class IdentityConfig
  ...
  def self.build_store(app_root:, rails_env:)
    ...
    Identity::Hostdata.load_config!(app_root:, rails_env:) do |config|
      ...
      config.add(:salesforce_consumer_key, type: :string, allow_nil: true)
      config.add(:salesforce_consumer_secret, type: :string, allow_nil: true)
      config.add(:salesforce_instance_url, type: :string, allow_nil: true)
