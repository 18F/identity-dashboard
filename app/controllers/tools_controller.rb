class ToolsController < ApplicationController
  ...
  def index
    ...
    begin
      certs = (get_cert_param || sp.certs).map { |cert| OpenSSL::X509::Certificate.new(cert) }
  ...
  def auth_params
    @auth_params ||= get_auth_request_params
  end

  def get_cert_param
    return [params['cert']] if params['cert'].length > 0
  end

  def get_auth_request_params
    auth_url = params['auth_url']
    return nil if auth_url.length < 1
    params = get_url_params( auth_url )
    params = {:SAMLRequest => auth_url} if params.length < 1
    return params
  end
