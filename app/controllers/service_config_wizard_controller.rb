...
class ServiceConfigWizardController < AuthenticatedController
  ...
  def validate_and_save_service_provider
    saver = ConfigSaver.new(draft_service_provider, current_user)
    saver.run

    return save_service_provider(draft_service_provider) if saver.errors.none?

    flash[:error] = saver.errors
  end
  ...
  def immediately_precedes?(current_step, previous_step)
    current_index = STEPS.index(current_step)
    previous_index = STEPS.index(previous_step)

    current_index && previous_index && (current_index + 1 == previous_index)
  end
end
