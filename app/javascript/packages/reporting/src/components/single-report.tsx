...
import {NEW_ROUTES} from "../routes/all";



const yearMonthDayParse = utcParse("%Y-%m-%d");

export type SingleReport = (props: ReportProps) => VNode;

interface ReportProps {
  path: keyof typeof NEW_ROUTES;
  start?: string;
  finish?: string;
  ial?: string;
  /**
...
function wrapSingleReport(
  ...
  return ({
    ...
    const start =
      (startParam && yearMonthDayParse(startParam)) || startOfPreviousWeek;
    const finish =
      (finishParam && yearMonthDayParse(finishParam)) || endOfPreviousWeek;
...
export default wrapSingleReport;
