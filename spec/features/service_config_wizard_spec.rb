...
feature 'Service Config Wizard' do
  ...
  context 'when Login.gov admin' do
    ...
    it 'shows an error and does not crash when a non-PEM file is uploaded as a cert' do
      ...
      expect(page).to_not have_content('Null Certificate')
    end
    ...
    describe 'and Production gate is enabled' do
      ...
      it 'allows creation of an application' do
        ...
        help_text = {
          'sign_in' => { 'en' => 'hello', 'es' => 'hola', 'fr' => 'bonjour', 'zh' => '你好' },
          'sign_up' => { 'en' => 'hello', 'es' => 'hola', 'fr' => 'bonjour', 'zh' => '你好' },
          'forgot_password' => { 'en' => 'hello', 'es' => 'hola', 'fr' => 'bonjour',
                                 'zh' => '你好' },
        }
        visit new_service_config_wizard_path
        click_on 'Next' # Skip the intro page
  ...
  context 'when not login.gov admin' do
    ...
    describe 'starting at the service provider index' do
      ...
      it 'goes through the redirector for documentation links' do
        ...
        expect(destination_url).to eq(documentation_path(destination: 'production'))
      end
  ...
  context 'when selecting SAML' do
    ...
    describe 'Redirects are validated' do
      ...
      scenario 'invalid URLs should display an error' do
        ...
        redirects_input = page.find('[name="wizard_step[redirect_uris][]"]')
