...
feature 'Users can access service providers that belong to their team' do
  context 'Index' do
    ...
    scenario 'sandbox users can see all configs for their team' do
      ...
      members_prod_app = create(:service_provider, :with_prod_config, team: team0, user: user2)
      members_sandbox_app = create(:service_provider, :with_sandbox, team: team0, user: user2)
      ...
      expect(page).to have_content(members_prod_app.friendly_name)
      expect(page).to have_content(members_sandbox_app.friendly_name)
    ...
    scenario 'prod users can only see production configs for their team' do
      ...
      expect(page).to have_content(members_prod_app.friendly_name)
      expect(page).to_not have_content(members_sandbox_app.friendly_name)
      expect(page).to_not have_content(no_longer_a_member_app.friendly_name)
      expect(page).to_not have_content(other_app.friendly_name)
    end
  end
