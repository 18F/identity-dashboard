...
RSpec.describe WizardStep, type: :model do
  ...
  let(:random_form_step) { WizardStep::STEPS[1..].sample }
    ...
    it 'populates all properties for all steps' do
      WizardStep::STEPS.each do |step_name|
        ...
        WizardStep::STEP_DATA[step_name].fields.keys.each do |field_name|
          expect(subject.send(field_name)).to eq(
            WizardStep::STEP_DATA[step_name].fields[field_name],
          )
        end
      end
    end
  ...
  describe '#get_step' do
    ...
    it 'pulls the relevant step out of the database' do
      ...
      expected_result = create(:wizard_step, step_name: step_name_to_find, user:)
    ...
    it 'builds a new step if no matching step exists' do
      ...
      expected_result = WizardStep.find_or_initialize_by(step_name: step_name_to_find, user:)
  ...
  context 'step "authentication"' do
    ...
    describe '#valid?' do
      it 'validates good wizard_form_data' do
        expect(authentication_step).to be_valid, authentication_step.errors.full_messages.join
      end
      ...
      it 'fails with bad wizard_form_data' do
        ...
        expect(authentication_step).to_not be_valid
  ...
  context 'step "logo_and_cert"' do
    ...
    describe '#certificates' do
      ...
      context 'with an existing logo' do
        ...
        it 'does not replace a good logo with a bad logo' do
  ...
  describe '.all_step_data_for_user' do
    ...
    it 'stops returning wizard_form_data that have been deleted' do
