...
describe Reports do
  ...
  context 'using local files for test data' do
    ...
    it 'works with test data' do
      ...
      expect(subject.data['count_newly_proofed']).to eq(83)
      expect(subject.data['count_previously_proofed']).to eq(20)
    end
  end
  ...
  describe '.monthly_dates_since' do
    it 'returns monthly dates from last month back through start_date' do
      travel_to Date.new(2026, 3, 15) do
        expect(described_class.monthly_dates_since(Date.new(2026, 1, 1))).to eq(
          %w[2026-02-01 2026-01-01],
        )
      end
    end
