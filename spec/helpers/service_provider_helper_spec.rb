...
describe ServiceProviderHelper do
  ...
  describe '#show_minimal_help_text_element' do
    ...
    let(:blank_help_text) {
      double = double(HelpText.new)
      expect(double).to receive(:blank?).and_return(true)
      double
    }
    let(:filled_help_text) {
      double = double(HelpText.new)
      expect(double).to receive(:blank?).and_return(false)
      double
    }
    ...
    context 'logged in as a non admin user' do
      ...
      it 'returns false when text is not blank' do
        expect(helper.show_minimal_help_text_element?(filled_help_text)).to be false
      end

      it 'returns true when text is blank' do
        expect(helper.show_minimal_help_text_element?(blank_help_text)).to be true
      end
    end
    ...
    context 'as an admin user' do
      ...
      it 'returns false regrdless of the help text' do
        expect(helper.show_minimal_help_text_element?(filled_help_text)).to be false
        expect(helper.show_minimal_help_text_element?(blank_help_text)).to be false
      end
    end
  end
end
