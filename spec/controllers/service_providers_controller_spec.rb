  ...
  describe '#create' do
    ...
    context 'help_text config' do
      it('should fill selected default options: set 1') do
        ...
        help_params_1 = { sign_in: {en: sign_in_key}, sign_up: {en: sign_up_key} , forgot_password: { en: forgot_password_key} }
        post :create, params: { service_provider: { issuer: 'issuer-123', help_text: help_params_1, friendly_name: 'ABC', group_id: user.teams.first.id } }
        sp = ServiceProvider.find_by(issuer: 'issuer-123')
        expect(sp.help_text).to eq({
          'sign_in' => {
            'en' => I18n.t(
              "service_provider_form.help_text.sign_in.#{sign_in_key}",
              sp_name: sp.friendly_name,
              agency: sp.agency.name,
              locale: :en,
            ),
            'es' => I18n.t(
              "service_provider_form.help_text.sign_in.#{sign_in_key}",
