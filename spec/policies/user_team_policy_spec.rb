...
describe UserTeamPolicy do
  ...
  permissions :manage_team_users? do
    ...
    it 'allows team member to manage team users' do
      expect(UserTeamPolicy).to permit(without_role_membership.user, without_role_membership)
    end

    it 'allows admin to manage team users' do
      expect(UserTeamPolicy).to permit(site_admin, without_role_membership)
    end

    it 'does not allow a random user to manage team users' do
      expect(UserTeamPolicy).to_not permit(other_user, without_role_membership)
    end
  end

  permissions :index? do
    it 'allows Partner Admins' do
      expect(UserTeamPolicy).to permit(partner_admin, partner_admin_membership)
    end

    it 'allows Partner Developers' do
      expect(UserTeamPolicy).to permit(partner_developer, partner_developer_membership)
    end

    it 'forbids Partner Readonly' do
      expect(UserTeamPolicy).to_not permit(partner_readonly, partner_readonly_membership)
    end
  end
  ...
  permissions :create? do
    it 'allows Partner Admins' do
      ...
      expect(UserTeamPolicy).to permit(partner_admin, new_membership)
    end

    it 'forbids Partner Developers' do
      new_membership = partner_developer_membership.team.user_teams.build
      expect(UserTeamPolicy).to_not permit(partner_developer, new_membership)
    end

    it 'forbids Partner Readonly' do
      new_membership = partner_readonly_membership.team.user_teams.build
      expect(UserTeamPolicy).to_not permit(partner_readonly, new_membership)
    end
  end

  permissions :destroy? do
    it 'allows site admins' do
      expect(UserTeamPolicy).to permit(site_admin, partner_admin_membership)
    end
    ...
    it 'allows site admins to delete their own membership' do
      ...
      expect(UserTeamPolicy).to permit(site_admin, admin_team_membership)
    end
    ...
    it 'forbids Partner Admins on their own membership' do
      expect(UserTeamPolicy).to_not permit(partner_admin, partner_admin_membership)
    end
