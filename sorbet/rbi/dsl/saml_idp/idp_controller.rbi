# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for dynamic methods in `SamlIdp::IdpController`.
# Please instead update this file by running `bin/tapioca dsl SamlIdp::IdpController`.


class SamlIdp::IdpController
  include GeneratedUrlHelpersModule
  extend GeneratedUrlHelpersModule
  include GeneratedPathHelpersModule
  extend GeneratedPathHelpersModule

  sig { returns(HelperProxy) }
  def helpers; end

  module HelperMethods
    include ::ActionController::Base::HelperMethods
    include ::ApplicationHelper
    include ::BannerHelper
    include ::SecurityEventHelper
    include ::ServiceConfigWizardHelper
    include ::ServiceProviderHelper
    include ::TeamHelper
    include ::ToolHelper
    include ::UserHelper
    include ::DeviseHelper

    sig { returns(T.untyped) }
    def saml_acs_url; end
  end

  class HelperProxy < ::ActionView::Base
    include HelperMethods
  end
end
